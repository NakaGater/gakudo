...
export function ForgotPasswordForm() {
  const [state, action, pending] = useActionState<ActionState, FormData>(forgotPassword, null);
  ...
  return (
    <Card className="w-full max-w-sm">
      <CardHeader>
        <h1 className="text-xl font-bold text-center text-fg">パスワードをお忘れですか</h1>
      </CardHeader>
      <CardContent>
        <form action={action} className="flex flex-col gap-4">
          ...
          <Input label="メールアドレス" name="email" type="email" autoComplete="email" required />
